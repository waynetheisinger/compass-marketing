...
**Reference docs:** https://developer.ebay.com/docs

### Overview
eBay REST APIs for managing listings, orders, and inventory on the MowDirect eBay seller account.

### Authentication
Unlike Shopify and BaseLinker, eBay requires **User access tokens** (not application tokens) for all selling operations. These are obtained via a one-time OAuth browser flow and then refreshed automatically.

- **One-time setup:** run `scripts/ebay_auth.py` — opens browser, you log in as the MowDirect eBay seller, approve, paste the redirect URL back. Saves refresh token to `.env`.
- **Ongoing:** access tokens last 2 hours and are refreshed silently using the refresh token (~18 month lifespan). No user interaction needed after initial setup.
- **If refresh token expires:** re-run `scripts/ebay_auth.py`

### Key APIs

| API | Base path | Purpose |
|---|---|---|
| Fulfillment | `/sell/fulfillment/v1` | Orders, shipping, refunds, disputes |
| Inventory | `/sell/inventory/v1` | Listings, inventory items, offers |

### Base client module
All eBay scripts must use `scripts/ebay_client.py` — do not reimplement token refresh. Usage:
...
EBAY_RUNAME=...
EBAY_REFRESH_TOKEN=...   # generated by scripts/ebay_auth.py — do not set manually
```
